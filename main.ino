 ...
 * Based on the code of keebie81 -> https://www.instructables.com/Binary-Game/
 * ---------------------------------
Modified version - 24/08 by skuydi
 * replaced the liquid crystal display by 7 segement display's
 * added a countdown timer to make the game more dynamic
 * added some sounds
 * added a score calculation
 * added a neopixel ring to show the score
 * added more ranges for random numbers
*/  
...
// Display 2 - Initializing the LedControl object - Button state
// Parameters : DataIn, CLK, LOAD/CS, number of MAX7219 drivers connected
...
#define DURATION 5000 // Animation duration in milliseconds
...
// Range for random numbers in different modes
// Setup
int randEasy_1        = 15;   // Easy
int randNormal_1      = 25;   // Normal
int randMedium_1      = 35;   // Medium
int randHard_1        = 55;   // Hard
int randExtreme_1     = 85;   // Extreme
int randInsane_1      = 105;  // Insane
int randNightmare_1   = 128;  // Nightmare
int randImpossible_1  = 255;  // Impossible
...
int randExtreme_2     = 85;   // Extreme
int randInsane_2      = 105;  // Insane
int randNightmare_2   = 128;  // Nightmare
int randImpossible_2  = 255;  // Impossible
...
int randExtreme_3     = 85;   // Extreme
int randInsane_3      = 105;  // Insane
int randNightmare_3   = 128;  // Nightmare
int randImpossible_3  = 255;  // Impossible
...
int randExtreme_4     = 85;   // Extreme
int randInsane_4      = 105;  // Insane
int randNightmare_4   = 128;  // Nightmare
int randImpossible_4  = 255;  // Impossible

// Time limit for different modes
...
void setup() {
  ...
  display_2.shutdown(0, false);       // Switch on driver 0
  display_2.setIntensity(0, 8);       // Set brightness to medium level (0-15)
  display_2.clearDisplay(0);          // Clear display
...
void displayNumber(long number) {
    // Extract each digit from the number and display it on the screen
  for (int i = 0; i < 8; i++) {
    int digit = number % 10;                 // Extract the least significant digit
    display_2.setDigit(0, i, digit, false);  // Display the digit on the screen
    number /= 10;                            // Delete the least significant digit
  }
}
...
void rainbowCycle(uint8_t wait) {
  uint16_t i, j;
  unsigned long start = millis(); // Records start time

  while(millis() - start < DURATION) { // Continue animation for DURATION milliseconds
    for(j = 0; j < 256; j++) { // A complete rainbow cycle
      for(i = 0; i < strip.numPixels(); i++) {
        strip.setPixelColor(i, Wheel(((i * 256 / strip.numPixels()) + j) & 255));
      }
      strip.show();
      delay(wait);
      if(millis() - start >= DURATION) break; // If the duration is reached, exit the loop
    }
  }
}
...
void updateScore() {
  if (correctNumber == 1) {
    ...
    if (score > maxScore) score = maxScore;  // Limit score to maximum value

    if (score == maxScore) {
      score = 0;
      playVictoryMelody();  // Play victory melody if score reaches maximum value
      rainbowCycle(5);      // 20 ms delay between updates
    }
  }
  if (wrongNumber == 1) {
    score--;                   // Decrements the score if the answer is incorrect
    if (score < 0) score = 0;  // Limit score to 0

    if (score == 0) {
      playDefeatMelody();      // Play defeat melody if score reaches 0
    }
  }
}
...
void playVictoryMelody() {
  // Play the melody of victory// Joue la mélodie de victoire
...
void playDefeatMelody() {
        // Play the defeat melody
  int melody[] = { 262, 196, 196, 220, 196, 0, 247, 262 };  // Tone of notes
  int noteDurations[] = { 4, 8, 8, 4, 4, 4, 4, 4 };         //  Note duration
...
/*
// Function for mode selection (NOT USED)
